...
async function attemptLogin(username, email){
    ...
    if (code !== "email_already_exists") return user.createUser; // else don't really matter, client can handle.

    // login users
    const userInfo = await db.findOne("mathbunk", "users", {"email": email});
    console.log('user info: '+userInfo.email)
    return {succeded: true, code:"successful_login", reason:"Loged in."};
}
